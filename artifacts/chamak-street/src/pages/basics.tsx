...
import { Eye, LayoutGrid, Grid2X2, X } from "lucide-react";
...
export default function Basics() {
  ...
  const { data: rawProducts, isLoading } = useQuery<Product[]>({
    queryKey: ["basics-products", debouncedSearch],
    queryFn: async () => {
      const url = new URL(`${BASE}/api/products`, window.location.origin);
      url.searchParams.set("collection", "basics");
      ...
      <div className="min-h-screen bg-black">
        ...
        <div className="sticky top-[56px] md:top-[109px] z-30 border-b border-white/8 glass-nav">
          <div className="max-w-[1440px] mx-auto px-4 sm:px-6">
            <div className="flex flex-wrap items-center gap-2 sm:gap-3 py-2.5">
              {/* Search */}
          ...
          {isLoading ? (
            ...
            >
              ...
              <h3 className="text-xl font-black uppercase tracking-wider text-white mb-2">
                {debouncedSearch ? "No results" : "Coming Soon"}
              </h3>
              <p className="text-white/40 text-sm mb-7">
                {debouncedSearch
                  ? `Nothing matched "${debouncedSearch}" in Basics`
                  : "The Basics collection is being curated. Check back soon."}
              </p>
              <Link href="/shop">
                <motion.button
            ...
            <AnimatePresence mode="wait">
              <motion.div
                key={`${debouncedSearch}-${sortKey}-${cols}`}
