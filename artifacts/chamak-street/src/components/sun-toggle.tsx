...
export function SunToggle() {
  ...
  const [sun, setSun] = useState(!isLiquid);
  const tids = useRef<ReturnType<typeof setTimeout>[]>([]);

  const handleClick = () => {
    tids.current.forEach(clearTimeout);
    tids.current = [];
    const next = theme === "default" ? "liquid-glass" : "default";
    setTheme(next);
    const tid = setTimeout(() => setSun(next === "default"), 60);
  ...
  return (
    <button
      onClick={handleClick}
      aria-label={isLiquid ? "Switch to default theme" : "Switch to glass theme"}
      title={isLiquid ? "Default theme" : "Glass theme"}
      className="relative flex items-center justify-center w-12 h-12 rounded-full hover:bg-white/10 active:scale-90 transition-colors duration-150 cursor-pointer"
      ...
      <svg width="30" height="30" viewBox="0 0 24 24" style={{
        ...
        <defs>
          <mask id={maskId}>
            ...
            <circle cx="12" cy="12" r="5.7" fill="black" style={{
        ...
        {Array.from({ length: 8 }).map((_, i) => (
          <g key={i} transform={`rotate(${i * 45}, 12, 12)`}>
            <rect x={7.9} y={11} width={3.6} height={2} rx={1} fill={rayFill}
              style={{
                transformBox: "fill-box", transformOrigin: "left center",
                transform: sun ? "scaleX(1)" : "scaleX(0)", opacity: sun ? 1 : 0,
                transition: [
        ...
        <circle cx="12" cy="12" r="5.5" fill={bodyFill} mask={`url(#${maskId})`}
          style={{ transition: "fill 0.5s ease" }} />
        {[{ cx: 17.5, cy: 6.5, r: 0.7 }, { cx: 19.2, cy: 11, r: 0.5 }, { cx: 16.5, cy: 17, r: 0.6 }].map((s, i) => (
          <circle key={i} cx={s.cx} cy={s.cy} r={s.r} fill="rgba(200,225,255,0.9)"
            style={{
              opacity: sun ? 0 : 1, transform: sun ? "scale(0)" : "scale(1)",
